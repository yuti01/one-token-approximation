...
def verify_args(args):
    if args.pmin > args.pmax:
        raise ValueError(
            "pmin must be less than pmax, got pmin={}, pmax={}".format(
                args.pmin, args.pmax
            )
        )
    if args.smin > args.smax:
        raise ValueError(
            "smin must be less than smax, got smin={}, smax={}".format(
                args.smin, args.smax
            )
        )
    if (not args.word and not args.words) or (args.word and args.words):
        raise ValueError(
            "Either a single word or a file containing words must be given via --word or --words"
        )
    if args.prefix != "" and not args.prefix.endswith(" "):
        raise ValueError(
            'The prefix must either be empty or end with a space, got "{}"'.format(
                args.prefix
            )
        )
    if args.suffix != "" and not args.suffix.startswith(" "):
        raise ValueError(
            'The suffix must either be empty or start with a space, got "{}"'.format(
                args.suffix
            )
        )
...
def initialize_embeddings(
    tokens: List[List[str]], embeddings: List[List[np.ndarray]], strategy: str
) -> torch.Tensor:
    ...
    logger.info(
        "Initializing embeddings of shape {} x {} (strategy={})".format(
            batch_size, embedding_dim, strategy
        )
    )
    ...
    if strategy == "sum":
        ...
        for idx, token_embeddings in enumerate(embeddings):
            ...
            for token_idx, token in enumerate(word_tokens):
                embeddings_sum[idx] += torch.tensor(
                    token_embeddings[token_idx]
                ) * utils.token_length(token)
            embeddings_sum[idx] /= sum(
                utils.token_length(token) for token in word_tokens
            )
...
def main():
    ...
    parser.add_argument(
        "--eval_steps",
        default=[1, 10] + [100 * i for i in range(1, 51)],
        type=int,
        nargs="+",
    ...
    logger.info(
        "Inferring embeddings for {} words, first 10 are: {}".format(
            len(words), words[:10]
        )
    )
    ...
    model.embeddings.word_embeddings = OverwriteableEmbedding(
        word_embeddings, overwrite_fct=None
    )

    while True:
        batch = words_for_token_size[
        ...
        if not batch:
            if token_sizes_idx < len(token_sizes) - 1:
                ...
                batch_idx = 0
                logger.info(
                    "Processing all words that consist of {} tokens (found {} words, first 10 are: {})".format(
        ...
        embeddings = [
            [
                utils.get_word_embedding(wordpart, tokenizer, model, device)
                for wordpart in tokenize_with_optional_space(wrd)
            ]
            for wrd in batch
        ]
        ...
        input_gold, input_inference, index_to_optimize, layers_gold = (
            None,
            None,
            None,
            None,
        ...
        if not uses_randomization:
            (
                ...
                input_inference,
                index_to_optimize,
            ) = input_preparator.prepare_batch(batch)

            print("IGS:", input_gold.tokens.shape, "IIS", input_inference.tokens.shape)

            _, _, layers_gold = model(
                input_gold.tokens.to(device), input_gold.segments.to(device)
            )
        ...
        start = time.time()
        logger.info(
            " " * 79 + " ".join("{:6s}".format(word[:6]) for word in batch[:10])
        )
        ...
        for iteration in range(1, args.iterations + 1):
            ...
            _, _, layers = model(
                input_inference.tokens.to(device), input_inference.segments.to(device)
            )
            ...
            for idx in range(model.config.num_hidden_layers):
                ...
                if args.objective == "left" or args.objective == "both":
                    loss_val += loss(
                        layers[idx][:, :index_to_optimize, :],
                        layers_gold[idx][:, :index_to_optimize, :],
                    )

                if args.objective == "right" or args.objective == "both":
                    loss_val += loss(
                        layers[idx][:, index_to_optimize + 1 :, :],
                        layers_gold[idx][
                            :, index_to_optimize + len(embeddings[0]) :, :
                        ],
                    )
            ...
            if (iteration == 1 or iteration % 100 == 0) or do_eval:
                    ...
                    for idx, token_embeddings in enumerate(embeddings):
                        ...
                        if np.linalg.norm(inferred_embedding) > 0:
                            cosine_distance = scipy.spatial.distance.cosine(
                                token_embeddings[0], inferred_embedding
                            )
                ...
                cosine_string = " ".join(
                    ["{:6.2f}".format(dist) for dist in batch_dists][:10]
                )
                ...
                remaining_time_for_other_batches = (
                    (remaining_words / args.batch_size)
                    * args.iterations
                    / steps_per_second
                )
                remaining_time_for_this_batch = (
                    args.iterations - iteration
                ) / steps_per_second
                remaining_time = int(
                    remaining_time_for_this_batch + remaining_time_for_other_batches
                )

                logger.info(
                    "step: {:4d} loss: {:8.6f} cosine: {:6.2f} steps/s: {:6.2f} ETR: {:8s} sample: {}".format(
                        iteration,
                        loss_val.item(),
                        utils.avg(batch_dists),
                        steps_per_second,
                        utils.get_date_string(remaining_time),
                        cosine_string,
                    )
                )
        ...
        for idx, token_embeddings in enumerate(embeddings):
            ...
            inferred_embedding = optim_vars[idx].cpu().detach().numpy()
            if len(embeddings[0]) == 1:
                final_cosine_distance = scipy.spatial.distance.cosine(
                    token_embeddings[0], inferred_embedding
                )
